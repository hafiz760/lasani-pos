  ...
  const generateReport = async (range: DateRange) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const netProfit =
          totals.totalSales - (totals.totalExpenses + totals.totalRefunds + totals.totalPurchases)
            ...
            reportRows.map((row) => (
              <TableRow key={row.id} className="hover:bg-muted/30 transition-colors">
                ...
                <TableCell className="text-right">
                  <span
                    className={`font-bold ${row.entryType === 'DEBIT' ? 'text-emerald-500' : 'text-red-500'}`}
                  >
                    {row.entryType === 'DEBIT' ? '' : '-'}
                ...
                <TableCell>
                  <Badge
                    variant="secondary"
                    className="text-[10px] uppercase font-black bg-muted/50 text-muted-foreground"
                  >
                    {row.referenceType}
                  </Badge>
                </TableCell>
