...
const DEFAULT_MONGODB_URI = 'mongodb://localhost:27017/pos_db'
...
export function DatabaseConfigModal({
  ...
  const [mongoUri, setMongoUri] = useState(DEFAULT_MONGODB_URI)
  ...
  const loadCurrentConfig = async () => {
    try {
      ...
      if (result.success && result.data?.MONGODB_URI) {
        ...
        setMongoUri(DEFAULT_MONGODB_URI)
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(result.message || 'Database configuration saved and connected.')
        onSuccess?.()
        onClose()
      } else {
      ...
      >
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              id="mongoUri"
              placeholder={DEFAULT_MONGODB_URI}
              value={mongoUri}
              onChange={(e) => {
            ...
            <p className="text-xs text-muted-foreground">
              Enter your MongoDB connection URI. The default local RexPOS database is:
              mongodb://localhost:27017/pos_db
            </p>
          </div>
            ...
            <AlertDescription>
              <div className="space-y-2 text-sm">
                ...
                <ul className="list-disc list-inside space-y-1 text-xs">
                  <li>
                    Local:{' '}
                    <code className="bg-muted px-1 rounded">mongodb://localhost:27017/pos_db</code>
                  </li>
                  <li>
                    Network:{' '}
                    <code className="bg-muted px-1 rounded">
                      mongodb://192.168.1.100:27017/lasani-pos
                    </code>
                  </li>
                  <li>
                    With Auth:{' '}
                    <code className="bg-muted px-1 rounded">
                      mongodb://user:pass@localhost:27017/lasani-pos
                    </code>
                  </li>
                </ul>
              </div>
        ...
        <div className="flex justify-between gap-2">
          <Button variant="outline" onClick={handleClose} disabled={!canClose || saving}>
