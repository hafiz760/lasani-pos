...
export function ReportPage({
  ...
  const activeRangeLabel = useMemo(() => {
    ...
    if (
      !reportRange.to ||
      startOfDay(reportRange.from).getTime() === startOfDay(reportRange.to).getTime()
    ) {
      return fromLabel
    }
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="space-y-2">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className="h-11 border-border min-w-[240px] justify-start px-4"
                  >
              ...
              {quickRanges.map((quick) => (
                <Button
                  ...
                  className={`h-9 px-4 text-xs font-bold transition-all duration-200 ${
                    selectedRangeLabel === quick.label
      ...
      <Card className="border-border overflow-hidden">
        ...
        <CardContent className="p-0">
          <ScrollArea className="h-fit max-h-[600px]">{children}</ScrollArea>
        </CardContent>
      </Card>
    </div>
  )
