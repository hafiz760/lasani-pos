...
import { reconnectToDatabase } from '../../lib/mongodb'
...
export function registerConfigHandlers() {
  ipcMain.handle('config:seedStarterData', async () => {
    try {
      if (mongoose.connection.readyState !== 1) {
        return {
          success: false,
          error: 'Connect to a database before running the starter data seed.'
        }
      }

      await seedInitialData()
      return { success: true, message: 'Starter data has been verified.' }
    } catch (error: any) {
      return { success: false, error: error.message }
    }
  })
  ...
  ipcMain.handle('config:save', async (_event, data) => {
    try {
      const mongoUri = typeof data?.MONGODB_URI === 'string' ? data.MONGODB_URI.trim() : ''
      if (!mongoUri) {
        return { success: false, error: 'A MongoDB connection string is required.' }
      }

      const testConnection = await mongoose.createConnection(mongoUri).asPromise()
      await testConnection.close()

      configManager.saveConfig({ MONGODB_URI: mongoUri, isConfigured: true })
      const result = await reconnectToDatabase()

      if (!result.success) {
        return {
          success: false,
          error: result.error || 'Unable to connect using the saved configuration.'
        }
      }
      ...
      const message =
        result.source === 'development-environment'
          ? 'Database configuration saved. The development MONGODB_URI environment override remains active.'
          : 'Database configuration saved and connected.'
